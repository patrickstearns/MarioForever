            ...
            if (canEnterLevel && keys[Mario.KEY_JUMP]){
                if (level[x][y] == TILE_LEVEL && data[x][y] == -11){}
                    ...
                    if (level[x][y] == TILE_LEVEL && data[x][y] != 0 && data[x][y] > -10){
                        ...
                        LevelGenerator.Background type = LevelGenerator.Background.HILLS;
                        if (data[x][y] > 1 && new Random(seed + x * 313211 + y * 534321).nextInt(3) == 0)
                            type = LevelGenerator.Background.UNDERGROUND;
                        if (data[x][y] < 0){
                            if (data[x][y] == -2){
                                Mario.levelString += "X";
                                difficulty += 2;
                            }
                            else if (data[x][y] == -1) Mario.levelString += "?";
                            else{
                                Mario.levelString += "#";
                                difficulty += 1;
                            }

                            type = LevelGenerator.Background.CASTLE;
                        }
                        else Mario.levelString += data[x][y];

                        Art.stopMusic();
                        marioComponent.startRandomLevel(seed * x * y + x * 31871 + y * 21871, difficulty, type);
                    }
                }
            }
